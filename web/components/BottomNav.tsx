...
export default function BottomNav() {
  ...
  // Nếu không đăng nhập, không hiển thị
  if (!isAuthenticated) {
    return null
  }
  ...
  // Chọn menu items dựa trên viewMode
  const navItems = viewMode === 'agent' ? agentNavItems : customerNavItems
    ...
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 md:hidden safe-area-inset-bottom">
      <div className="flex justify-around items-center h-16 px-2">
        {navItems.map((item) => {
          ...
          // - Normalize pathname để loại bỏ trailing slash (trừ '/')
          const normalizedPathname = pathname.endsWith('/') && pathname !== '/' ? pathname.slice(0, -1) : pathname
          
          let isActive = false
          
          // - Nếu href là '/', chỉ active khi pathname chính xác là '/'
          if (item.href === '/') {
            isActive = normalizedPathname === '/'
          }
          // - Nếu href là '/agent', chỉ active khi pathname chính xác là '/agent' (không match với /agent/orders, /agent/products, etc.)
          else if (item.href === '/agent') {
            isActive = normalizedPathname === '/agent'
          }
          // - Nếu href khác, active khi pathname === href hoặc pathname.startsWith(href + '/')
          else {
            isActive = normalizedPathname === item.href || normalizedPathname.startsWith(item.href + '/')
          }
          
          const badgeValue = 'badge' in item ? (item as any).badge : undefined
              ...
              <div className="relative">
                ...
                {badgeValue && badgeValue > 0 && (
                  <span className={`absolute -top-2 -right-2 text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center ${
                    item.href === '/orders' || item.href === '/agent/orders' ? 'bg-red-600' : 'bg-primary-600'
                  }`}>
