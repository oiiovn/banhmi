<?php
/**
 * Webhook endpoint cho auto-deploy Web từ Git
 * Đặt file này trên hosting và setup webhook từ GitHub/GitLab
 * 
 * URL: https://websi.vn/deploy-web-webhook.php
 */
...
$DEPLOY_SCRIPT = '/home/username/domains/websi.vn/repo/deploy-web-auto.sh';  // ← Sửa đường dẫn này

// Log file
$LOG_FILE = '/home/username/domains/websi.vn/repo/deploy-webhook.log';  // ← Sửa đường dẫn này
...
if (isset($headers['X-Hub-Signature-256'])) {
    ...
    // Nếu không có signature, có thể bỏ qua (không an toàn nhưng để test)
    log_message('WARNING: No signature found, proceeding anyway');
}
...
log_message("Starting Web deployment for branch: $branch");
...
if (file_exists($DEPLOY_SCRIPT)) {
    // Chạy script trong background để không timeout
    $command = "bash $DEPLOY_SCRIPT >> $LOG_FILE 2>&1 &";
    exec($command);
    
    log_message('Deployment script started in background');
    http_response_code(200);
    echo json_encode([
        'status' => 'success',
        'branch' => $branch,
